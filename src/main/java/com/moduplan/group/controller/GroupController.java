...
public class GroupController {
    ...
    public ResponseEntity<ApiResponse<GroupDetailResponse>> getGroupDetail(
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponse.success(200, "모임 상세 조회에 성공했습니다.", response));
    }

    @Operation(summary = "내 모임 목록 조회")
    @GetMapping("/me")
    public ResponseEntity<ApiResponse<MyGroupPageResponse>> getMyGroups(
            Authentication authentication,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size
    ) {
        ...
        Long userId = (Long) authentication.getPrincipal();
        MyGroupPageResponse response = groupService.getMyGroup(userId, PageRequest.of(page, size));
